...
def main():
    file_path = "CareProviders/narodni-registr-poskytovatelu-zdravotnich-sluzeb.csv"
    data = load_data(file_path)
    data_cube = as_data_cube(data)
    data_cube = create_hierarchy(data_cube, data)
    if not os.path.exists("output"):
        os.makedirs("output")
    with open("output/CareProviders.ttl", "wb") as file:
...
def create_hierarchy(cube: Graph, data):
    eurovoc_regauth = URIRef("http://eurovoc.europa.eu/6034")

    cube.add((eurovoc_regauth, RDF.type, SKOS.ConceptScheme))
    cube.add((eurovoc_regauth, SKOS.prefLabel, Literal("správní celek", lang="cs")))
    cube.add((eurovoc_regauth, SKOS.prefLabel, Literal("regional and local authorities", lang="en")))
    cube.add((eurovoc_regauth, SKOS.notation, Literal("6034")))
